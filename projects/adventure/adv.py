...
def traverseMap(player, direction = ''):
    ...
    if player.currentRoom.id not in graph:
        # Initialize in your room with '' exits
    ...
    if direction is not '':
        ...
        opposite = reverse_directions[direction]
        # set previous room using getRoomInDirection
        prev_room = player.currentRoom.getRoomInDirection(opposite)
        # update graph with previous room
        graph[currentRoom][opposite] = prev_room.id
    ...
    for exit in player.currentRoom.getExits():
        if graph[currentRoom][exit] == '?':
            # if current room has an unexplored, set new direction to that exit
            new_direction = exit
            # travel to the next and append current exit to the path
            player.travel(exit)
            traversalPath.append(exit)
            # set new room to players current room and set previous rooms exit to new room
    ...
    if new_direction is '?':
        # setup new queue with the currentRoom
        q = Queue()
        visited = set()
        ...
        while q.size() > 0:
            ...
            if currentRoom not in visited:
                ...
                # if current room has an unexplored exit
                if '?' in graph[currentRoom].values():
                    # return path to that room and reset queue
                    travel_path = path
                    q = Queue()
                    break
                
                for neighbor in graph[currentRoom].values():
                    # for direction in current room - add to the path to search through
                    new_path = list(path)
    ...
    for r in travel_path:
        ...
        for d in g_keys:
            # for every room we saw, add the values that match to that room to our path
            if graph[room][d] == r:
                player.travel(d)
